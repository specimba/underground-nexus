...
RUN echo "docker exec workbench bash /workbench.sh || true" >> deploy-olympiad.sh
RUN echo "docker exec workbench sudo apt install firefox -y || true" >> deploy-olympiad.sh
RUN echo "docker run -itd  --privileged --name=workbench -h workbench -e PUID=1000 -e PGID=1000 -e TZ=America/Colorado -p 1000:3000 --dns=10.20.0.20 --net=Inner-Athena --restart=always -v /dev:/dev -v workbench0:/config -v /nexus-bucket:/config/Desktop/nexus-bucket -v /var/run/docker.sock:/var/run/docker.sock workbench:latest" >> deploy-olympiad.sh || true
...
RUN echo "docker exec workbench echo "docker exec workbench sudo bash github-desktop.sh" >> /nexus-bucket/workbench.sh" >> deploy-olympiad.sh
...
RUN echo "docker exec workbench echo "docker exec workbench sudo apt install -y terminator" >> /nexus-bucket/workbench.sh" >> deploy-olympiad.sh
...
RUN echo "docker exec workbench echo "docker exec workbench sudo apt install -y virt-manager firefox" >> /nexus-bucket/workbench.sh" >> deploy-olympiad.sh
#------------------------------------
...
RUN echo "docker run -itd --privileged -p 9000:9000 -p 9010:9001 --name=torpedo -h torpedo --dns=10.20.0.20 --net=Inner-Athena --restart=always -v /nexus-bucket:/nexus-bucket -v /nexus-bucket/s3-torpedo:/data pgsty/minio:latest server /data --console-address ":9001"" >> deploy-olympiad.sh
...
RUN echo "docker run -itd -p 8200:1234 --name=Nexus-Secret-Vault -h Nexus-Secret-Vault --dns=10.20.0.20 --net=Inner-Athena --restart=always --cap-add=IPC_LOCK -e 'VAULT_DEV_ROOT_TOKEN_ID=myroot' -e 'VAULT_DEV_LISTEN_ADDRESS=0.0.0.0:1234' vault:1.13.3" >> deploy-olympiad.sh
...
#RUN echo "curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/arm64/kubectl" && chmod +x ./kubectl && mv ./kubectl /usr/local/bin/kubectl" >> deploy-olympiad.sh || true
RUN echo "k3d kubeconfig merge KuberNexus --kubeconfig-merge-default" >> deploy-olympiad.sh || true
RUN echo "docker exec Athena0 bash /nexus-bucket/underground-nexus/'Dagger CI'/Scripts/virtual-machine-engine.sh" >> deploy-olympiad.sh
